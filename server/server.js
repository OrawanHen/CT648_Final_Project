...
app.get('/api/get/historypoint', async (req, res) => {
    const client = await pool.connect();
    try {
        const result = await client.query(`SELECT * FROM oh_quiz_history`);
        return res.json(result.rows);
...
app.get('/api/quiz_login/quiz_login', async (req, res) => {
    ...
    try {
        const result = await client.query(
            'SELECT * FROM oh_quiz_login WHERE username = $1 AND password = $2',
            [username, password]
        );
        return res.json(result.rows);
    } catch (err) {
...
app.post('/api/questions', async (req, res) => {
    ...
    try {
        ...
        const result = await client.query(
            'INSERT INTO oh_quiz_title (title , create_date) VALUES ($1 ,$2) RETURNING id',
            ["GAME CREATE", "now()"]
        );
        ...
        for (const questions of questionsState) {
            ...
            const result = await client.query(
                'INSERT INTO oh_quiz_questions (title_id, question, category, difficulty, type, correct_answer, incorrect_answers) VALUES ($1, $2, $3, $4, $5, $6, $7)',
...
app.post('/api/singupuser', async (req, res) => {
    ...
    try {
        const checkResulu = await client.query(
            'select * from oh_quiz_login where username = $1',
            [userState.username]
        )
        if (checkResulu.rowCount > 0) {
            ...
            // Insert into the quiz_title table using PostgreSQL's parameterized queries
            const result = await client.query(
                'INSERT INTO oh_quiz_login (username , password , email) VALUES ($1 ,$2 , $3) ',
...
app.post('/api/gamepoint', async (req, res) => {
    ...
    try {
        // Insert into the quiz_title table using PostgreSQL's parameterized queries
        const result = await client.query(
            'INSERT INTO oh_quiz_history (gamepoint , useremail , gamename , time) VALUES ($1 ,$2 , $3 , $4) ',
...
app.delete('/api/deletequestion/:id', async (req, res) => {
    ...
    console.log('id', id)
    try {
        const result = await client.query(`DELETE FROM oh_quiz_title WHERE id = $1`, [id]);
        console.log('result', result)
        if (result.rowCount > 0) {
            const result = await client.query(`DELETE FROM oh_quiz_questions WHERE title_id = $1`, [id]);
            if (result.rowCount > 0) {
                res.status(201).json({
